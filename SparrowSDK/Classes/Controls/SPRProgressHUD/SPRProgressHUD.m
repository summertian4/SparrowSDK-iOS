...
#define SPRMainThreadAssert() NSAssert([NSThread isMainThread], @"SPRProgressHUD needs to be accessed on the main thread.");

CGFloat const SPRProgressMaxOffset = 1000000.f;

static const CGFloat SPRDefaultPadding = 4.f;
static const CGFloat SPRDefaultLabelFontSize = 16.f;
static const CGFloat SPRDefaultDetailsLabelFontSize = 12.f;


@interface SPRProgressHUD () {
    // Deprecated
...
@interface SPRProgressHUDRoundedButton : UIButton
@end


@implementation SPRProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SPRProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SPRProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (SPRProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            SPRProgressHUD *hud = (SPRProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = SPRProgressHUDAnimationFade;
    _mode = SPRProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(SPRProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SPRProgressHUDAnimationZoom) {
        type = animatingIn ? SPRProgressHUDAnimationZoomIn : SPRProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SPRProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SPRProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SPRProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SPRProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    SPRProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SPRProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SPRBackgroundView *backgroundView = [[SPRBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SPRProgressHUDBackgroundStyleSolidColor;
    ...
    SPRBackgroundView *bezelView = [SPRBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:SPRDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:SPRDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [SPRProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:SPRDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SPRRoundProgressView class]];

    SPRProgressHUDMode mode = self.mode;
    if (mode == SPRProgressHUDModeIndeterminate) {
    ...
    else if (mode == SPRProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SPRBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SPRProgressHUDModeDeterminate || mode == SPRProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SPRRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SPRProgressHUDModeAnnularDeterminate) {
            [(SPRRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == SPRProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SPRProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SPRProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SPRProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SPRRoundProgressView class]]) {
        SPRRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SPRRoundProgressView appearanceWhenContainedIn:[SPRProgressHUD class], nil];
#else
        appearance = [SPRRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SPRProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SPRRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SPRRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SPRBarProgressView class]]) {
        SPRBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SPRBarProgressView appearanceWhenContainedIn:[SPRProgressHUD class], nil];
#else
        appearance = [SPRBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SPRProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SPRBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SPRBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SPRBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? SPRDefaultPadding : 0.f;
...
- (void)setMode:(SPRProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SPRProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation SPRRoundProgressView
...
@implementation SPRBarProgressView
...
@implementation SPRBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = SPRProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = SPRProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SPRProgressHUDBackgroundStyle)style {
    if (style == SPRProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = SPRProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SPRProgressHUDBackgroundStyle style = self.style;
    if (style == SPRProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SPRProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SPRProgressHUDRoundedButton
